...
- **`Ping`だけはQuantLib専用ポリシーの例外として恒久的に残す**。理由:
  xlwings配線(Import Functions/ブックのVBAプロジェクト)自体の健全性を、
  重い計算UDF(`FairRate`等)と切り離して確認するための軽量ヘルスチェック
  として意図的に追加。`#NAME?`/「オブジェクトが必要です」が出た際に、
  問題がファイル配線側かロジック側かを即座に切り分けられる。
